...
import { Activity, Star, Users, Trophy, Target, Zap, Crown, ArrowRight } from 'lucide-react'
...
export default function Dashboard() {
  ...
  // Use english data for consistent length/filter calculations
  const expEn = EXPERIENCE.en
  const projEn = PROJECTS.en
  ...
  const cloudLevel = Math.min(99, 50 + (cloudCount * 8)) 
  
  const designCount = expEn.filter(e => e.role.includes('Design')).length + projEn.filter(p => p.category.includes('Design')).length
  const designLevel = Math.min(99, 60 + (designCount * 10))

  const eventCount = expEn.filter(e => e.role.includes('Coordinator') || e.highlights.some(h => h.includes('Event'))).length + projEn.filter(p => p.category.includes('Event')).length
  const eventLevel = Math.min(99, 55 + (eventCount * 12))

  const videoCount = expEn.filter(e => e.role.includes('Video')).length + projEn.filter(p => p.category.includes('Video')).length
  const videoLevel = Math.min(99, 50 + (videoCount * 15))

  const webCount = projEn.filter(p => p.category.includes('Web') || p.category.includes('Cloud')).length
  const webLevel = Math.min(99, 45 + (webCount * 10))

  return (
    <section className="relative py-12 h-full flex flex-col justify-center max-w-5xl mx-auto w-full">
      <motion.div
      ...
      >
        <Link to="/about" className="liquid-glass px-6 py-4 flex items-center gap-4 group hover:bg-accent/10 transition-colors no-underline">
          <div className="text-right">
            <span className="block text-[10px] font-bold uppercase tracking-widest text-foreground/50 mb-1">{t('btn.next')}</span>
            <span className="block text-lg font-black uppercase text-foreground">{t('nav.about')}</span>
          </div>
